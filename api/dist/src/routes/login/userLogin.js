...
userLogin.post("/register", async (req, res) => {
    ...
    const isRedundantEmail = await db.pool.query("SELECT * FROM users WHERE email = $1", [emailLowerCase]);
    if (!user_name ||
        typeof user_name !== "string" ||
        user_name.trim().length === 0) {
        res.status(400).json({
            error: "Username is required and must only consist of alfabetical characters",
        });
        return;
    }
    ...
    try {
        ...
        const insertUser = await db.pool.query("INSERT INTO users (email, password, user_name) VALUES ($1, $2, $3) RETURNING *", [emailLowerCase, hashedPassword, user_name]);
...
userLogin.post("/login", async (req, res) => {
    ...
    try {
        const userAccount = await db.pool.query("SELECT * FROM users WHERE email = $1", [lowerCaseEmail]);
