 ...
 */
driverRoute.patch("/register_to_truck/:driver_id/:truck_id", async (req, res) => {
    ...
    try {
        const result = await db.pool.query(`UPDATE drivers
       SET truck_id = $1
       WHERE id = $2
       RETURNING *;`, [parsedTruckId, parsedDriverId]);
...
driverRoute.patch("/load_package/:package_id/:truck_id", async (req, res) => {
    ...
    try {
        const existing_package = await db.pool.query(`SELECT * FROM package WHERE id = $1`, [package_id]);
        const existing_truck = await db.pool.query(`SELECT * FROM trucks WHERE id = $1`, [truck_id]);
        if (existing_package.rowCount === 0 || existing_truck.rowCount === 0) {
            res.status(404).json({
                message: "the package or truck you are trying to register could not be found in the database",
            });
            return;
        }
